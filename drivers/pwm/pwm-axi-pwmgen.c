			      ...
			      unsigned long timeout __always_unused)
{
	...
	clk_period_ps = DIV_ROUND_CLOSEST_ULL(NSEC_PER_USEC, rate);
	cnt = axi_pwmgen_read(pwmgen, AXI_PWMGEN_CHX_PERIOD(ch));
	cnt *= clk_period_ps;
	if (cnt)
		capture->period = DIV_ROUND_CLOSEST_ULL(cnt, NSEC_PER_USEC);
	else
		capture->period = 0;
	cnt = axi_pwmgen_read(pwmgen, AXI_PWMGEN_CHX_DUTY(ch));
	cnt *= clk_period_ps;
	if (cnt)
		capture->duty_cycle = DIV_ROUND_CLOSEST_ULL(cnt, NSEC_PER_USEC);
	else
...
static int axi_pwmgen_remove(struct platform_device *pdev)
{
	...
	pwmchip_remove(&pwm->chip); // upstream returns void now
	return 0; // todo: any check needed?
}
