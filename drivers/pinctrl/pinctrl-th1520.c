					 ...
					 unsigned int *num_maps)
{
	...
	for_each_available_child_of_node_scoped(np, child) {
		...
		if (npins <= 0) {
			dev_err(dev, "no pins selected for %pOFn.%pOFn\n", np, child);
			return -EINVAL;
		}
		nmaps += npins;
	...
	for_each_available_child_of_node_scoped(np, child) {
		...
		if (ret) {
			dev_err(dev, "%pOFn.%pOFn: error parsing pin config\n", np, child);
			goto free_map;
		}
		...
		if (of_property_read_string(child, "function", &funcname)) {
			...
			if (!muxtype) {
				dev_err(dev, "%pOFn.%pOFn: unknown function '%s'\n",
					np, child, funcname);
				ret = -EINVAL;
				goto free_configs;
			}

			funcname = devm_kasprintf(dev, GFP_KERNEL, "%pOFn.%pOFn", np, child);
			if (!funcname) {
				ret = -ENOMEM;
				goto free_configs;
			}
			...
			npins = of_property_count_strings(child, "pins");
			pgnames = devm_kcalloc(dev, npins, sizeof(*pgnames), GFP_KERNEL);
			if (!pgnames) {
				ret = -ENOMEM;
				goto free_configs;
			}
		...
		of_property_for_each_string(child, "pins", prop, pinname) {
			...
			if (i == thp->desc.npins) {
				nmaps = rollback;
				dev_err(dev, "%pOFn.%pOFn: unknown pin '%s'\n",
					np, child, pinname);
				goto free_configs;
			}
		...
		if (funcname) {
			ret = pinmux_generic_add_function(pctldev, funcname, pgnames,
							  npins, (void *)muxtype);
			if (ret < 0) {
				dev_err(dev, "error adding function %s\n", funcname);
				goto free_map;
			}
		}
	}
